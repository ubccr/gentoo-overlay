...
inherit cmake python-single-r1 udev systemd tmpfiles
...
src_configure() {
	local mycmakeargs=(
		-DCMAKE_INSTALL_SYSCONFDIR=/usr/share/rdma
...
src_install() {
	...
	dotmpfiles "${FILESDIR}/tmpfiles.d/rdma.conf"

	insinto /usr/share/rdma

	udev_dorules "${ED}"/usr/share/rdma/udev/rules.d/70-persistent-ipoib.rules
	rm -r "${ED}"/usr/share/rdma/{udev,init.d} || die
