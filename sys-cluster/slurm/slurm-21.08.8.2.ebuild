...
inherit autotools bash-completion-r1 lua-single pam prefix toolchain-funcs systemd ${INHERIT_GIT} tmpfiles
...
LICENSE="GPL-2"
SLOT="0"
IUSE="debug hdf5 html ipmi pmi pmi2 json lua multiple-slurmd +munge mysql netloc numa ofed pam slurmdbd static-libs ucx torque X"
...
PATCHES=(
	"${FILESDIR}"/${PN}-21.08.8.2_autoconf-curl.patch
)
...
src_prepare() {
	...
	hprefixify auxdir/{ax_check_zlib,x_ac_{lz4,ofed,munge,hwloc,netloc,json}}.m4
	eautoreconf
}
...
src_configure() {
	...
	local myconf=(
		...
		--prefix=/usr/share/slurm
		--with-hwloc="${EPREFIX}/usr"
		--htmldir="${EPREFIX}/usr/share/doc/${PF}"
	)
	use pam && myconf+=( --with-pam_dir=/usr/$(getpam_mod_dir) )
...
src_compile() {
	...
	use pam && emake -C contribs/pam_slurm_adopt
	use pmi && emake -C contribs/pmi
	use pmi2 && emake -C contribs/pmi2
...
src_install() {
	...
	use pam && emake DESTDIR="${D}" -C contribs/pam_slurm_adopt install
	use pmi && emake DESTDIR="${D}" -C contribs/pmi install
	use pmi2 && emake DESTDIR="${D}" -C contribs/pmi2 install
	if use torque; then
